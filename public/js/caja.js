$(document).ready(function () {
      ...
      if ($("#totalRetirado").length === 0) {
        $("#resumenTotales").append(`
                  <div class="total-card retirado">
                      ...
                      <div class="total-valor" id="totalRetirado">$${totalRetiros.toLocaleString(
                        "es-CL"
                      )}</div>
                  </div>
              `);
  ...
  function cargarCaja() {
    ...
    console.log("📦 Mock: cargando caja...");

    // Simula respuesta de apertura/cierre
    setTimeout(() => {
      const resCaja = {
        monto_inicial: 50000,
      ...
      const fecha = new Date(resCaja.fecha_apertura);
      ...
      <div class="card shadow-sm border-primary">
        <div class="card-body">
          ...
          <p class="mb-1"><strong>Fecha:</strong> ${fechaFormateada} &nbsp; <strong>Hora:</strong> ${
        resCaja.hora_apertura
      }</p>
    ...
    `;
      ...
      // Simula respuesta de movimientos
      setTimeout(() => {
        const resMovimientos = {
          success: true,
          movimientos: [
            {
              id: 1,
            ...
            },
            {
              ...
              fecha: new Date().toISOString(),
              hora: "12:00:00",
              nombre_servicio: "Retiro",
              medio_pago: "Retiro Efectivo",
              monto: -5000,
              nombre_usuario: nombreUsuario,
            },
          ],
        ...
        if (!resMovimientos.success) {
          $("#tablaCaja tbody").html(
            '<tr><td colspan="9" class="text-center text-danger">Error al cargar movimientos (mock).</td></tr>'
          );
          calcularTotales([], montoInicial);
          return;
        }

        if (!resMovimientos.movimientos.length) {
          $("#tablaCaja tbody").html(
            '<tr><td colspan="9" class="text-center text-muted">No hay movimientos registrados (mock).</td></tr>'
          );
          calcularTotales([], montoInicial);
          return;
        }
        ...
        const filas = resMovimientos.movimientos
          .map((m) => {
            ...
            <tr>
              ...
              <td>${m.medio_pago}</td>
              <td class="${claseMonto}">${simbolo}$${montoMostrar.toLocaleString(
              "es-CL"
            )}</td>
  ...
  $("#formInicioCaja").on("submit", function (e) {
    ...
    function obtenerNumeroCaja() {
      return new Promise((resolve, reject) => {
        console.log("📦 Mock: Obteniendo número de caja...");

        setTimeout(() => {
          // Simula éxito
          const success = true; // cambia a false para simular error

          if (success) {
            const numero_caja = Math.floor(Math.random() * 10) + 1; // número de caja aleatorio entre 1 y 10
            console.log("✅ Mock: Número de caja obtenido:", numero_caja);
            resolve(numero_caja);
          } else {
            console.error("❌ Mock: Número de caja no disponible");
            reject("Número de caja no disponible (mock)");
          }
        }, 500); // medio segundo para simular la latencia
      });
    }

    // Función principal para abrir la caja
    async function abrirCajaMock() {
      try {
        ...
        console.log("📦 Mock: Enviando datos de apertura de caja...");
        console.table({
          numero_caja,
          id_usuario_apertura,
          fecha_apertura,
        ...
        setTimeout(() => {
          ...
          if (res.success) {
            ...
            console.log("✅ Mock: Caja abierta correctamente");

            Swal.fire({
              icon: "success",
              title: "¡Caja abierta!",
              text: "Caja abierta correctamente (mock)",
              timer: 2000,
        ...
        }, 1000); // espera de 1s para simular la petición
      } catch (error) {
        console.error("Error:", error);
        Swal.fire({
          icon: "error",
          title: "Error inesperado",
          text: error.message || "Ocurrió un error inesperado (mock)",
        });
      }
    }
  ...
  function realizarCierreCaja(idUsuarioCierre) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        const data = { success: true, msg: "Mock: caja cerrada correctamente" };
  ...
  $("#formRetiroEfectivo").on("submit", function (e) {
    ...
    if (monto > balanceActual) {
      Swal.fire({
        ...
        text: `No puede retirar más del efectivo disponible. Disponible: $${balanceActual.toLocaleString(
          "es-CL"
        )}, Intenta retirar: $${monto.toLocaleString("es-CL")}`,
      });
      return;
    }
    ...
    Swal.fire({
      ...
      html: `¿Está seguro de retirar <strong>$${monto.toLocaleString(
        "es-CL"
      )}</strong>?`,
      icon: "question",
      showCancelButton: true,
